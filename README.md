...
In true form version we can see the results straightly but it takes much more logics to deal with the sign bit when addition and multiplication happen, so it has a larger resource usage and lower frequency (50Mhz at 16bit). However, it has the correct functional simulation and can be directly employed onto your FPGA board to see the correct results. This version also has a good organization and clear code comments in order to help you to understand. Project_2 branch is the 2's complement code version and has best performance but until now the functional simulation has not been completed yet. 
...
|----build.bat  
|----build.tcl  
`assets` dir contains some pictures used for this readme. The `srcs` dir contains the contraint files `constrs_1`, simulation files `sim_1`, and HDL files `sources_1`. The `weights` dir contains the weight coe files and bias files for ram/rom IP initialization.  `model` has the python file of implemented model and pth file. `dataset` is the cifar10 dataset. `examples` has 2 pictures and coes. `build.bat` and `build.tcl` are for building project.
...
## Examples
`examples` directory has 2 picture files in 'jpg' and 'coe'. 'plane.coe' and 'car.coe' are generated by Pytorch. You can make a initialization of 'bram0' using these files. all picture coes are generated in channel-major form. The pictures below show the expected result of `plane.coe`. The right one is the result of 10 classification result after quantization before softmax.  

<p align="center">
  <img src="/assets/cifar10.png" alt="plane result1" width="500"/>
  <img src="/assets/cifar10_result.png" alt="plane result2" width="500"/>
</p>
